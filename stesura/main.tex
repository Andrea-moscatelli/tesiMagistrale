...
L'idea alla base di questo lavoro è quella di creare delle \emph{mappe di stima della posa} umana dalle quali estrarre delle \emph{heatmaps} globali e delle \emph{predizioni di pose}. Una volta ottenute queste due componenti sarà necessario combinare le loro caratteristiche temporali e spaziali di modo da identificare l'azione svolta.

Più nel dettaglio, indicando con $\mathcal{Y}_k \in \{x,y\}$ le cordinate del $k$-esimo giunto corporeo, possiamo definire un'intera struttura corporea come $\mathcal{Y} = \{\mathcal{Y}_1, ... , \mathcal{Y}_k, ..., \mathcal{Y}_K\}$, dove $K$ è il numero totale dei tipi di giunti corporei considerati.

Un classificatore multiclasse $g_t^k$ viene quindi addestrato per predire la posizione del $k$-esimo giunto corporeo al passo $t$ e per ogni punto \textbf{z} dell'immagine la sua mappa di stima relativa al giunto $k$ sarà:
$$
...
Quello che abbiamo fatto è stato cioè trasformare un video in un sequenza di heatmaps e pose. 


\section{Actional-Structural Graph Convolutional Networks for Skeleton-based Action Recognition (2019)}
Seconda spiegazione.
\section{Vertex feature encoding and hierarchical temporal modeling in a spatial-temporal graph convolutional network for action recognition (2019)}
Terza spiegazione.
