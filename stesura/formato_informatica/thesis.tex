...
\begin{itemize}
\item \textbf{Livelli} - In prima battuta è stato scelto di utilizzare una rete neurale composta da una sola cella LSTM più un livello completamente connesso per l'inferenza finale e di incrementare il numero di livelli progressivamente fino al raggiungimento ottimale dei risultati.
\item \textbf{Hidden units} - Per quanto riguarda il numero di unità nascoste di ogni cella LSTM della rete, è stata utilizzata la seguente regola generale
$$
N_h = \frac{N_{samples}}{\alpha \cdot (N_{in} + N_{out})}
$$
dove $N_{samples}$ è il numero di esempi del \emph{training set}, $N_{in}$ e $N_{out}$ sono il numero di unità nascoste in entrata e in uscita rispettivamente e $\alpha$ è un moltiplicatore arbitrario, generalmente fra 2 e 10, che indica quanto generale vogliamo sia il nostro modello o, in altre parole, quanto vogliamo evitare l'\emph{overfitting}.
...
\item \textbf{Ottimizzatore} - L'ottimizzatore scelto è \emph{RMSProp} \cite{RMSProp} con un valore $\rho = 0.9$ ed un learning rate iniziale di $0.01$, decrementato fino a $0,001$ per la fase di fine tuning.
...
Come prima cosa, considerata la dimensione del dataset (56880 video), è stato scelto un sottoinsieme di azioni di modo da poter lavorare su una porzione più piccola, sufficientemente rappresentativa e di difficile categorizzazione. A questo scopo sono state scelte 8 azioni che avessero le seguenti caratteristiche:
\begin{itemize}
...
Questa porzione di dataset così creata è probabilmente
...
\section{FAI}
\begin{itemize}
\item scelta azioni iniziali e lancio primo giro di test con tutte le tecniche
\item risultati
\item combinazione
\item incremento livello
\item testing dropout
