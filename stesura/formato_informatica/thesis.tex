...
Uno dei dataset utilizzati in questo lavoro è NTU-RGB+D \cite{NTURGB} che, come vedremo più in dettaglio nel capitolo 5, propone due metodi di valutazione distinti: \emph{cross-subject} e \emph{cross-view}. I migliori risultati ottenuti sono riassunti in tabella \ref{tab:risultati4}.
...
Generalmente esistono due approcci principali per affrontare i problemi di rilevamento di più persone in un'immagine, stima della loro posa e \emph{segmentazione} (ovvero l'identificazione dei pixel che rappresentano ogni persona):
\begin{itemize}
\item \textbf{Approccio top-down} - l'approccio \emph{top-down} inizia con una fase di identificazione e localizzazione approssimativamente della posizione delle persone, delimitando il riquadro dell'immagine entro il quale sono contenute e continua con una fase di stima della posa o di separazione ``primo piano-sfondo" nell'area identificata. 
...
Questo algoritmo dopo essere stato allenato col famoso dataset COCO-2016 \cite{COCO-2016} (che fra molte altre cose contiene anche l'annotazione dei 17 keypoint di migliaia di persone), è riuscito a migliorarne l'\emph{AP} (average-precision) da 0,655 a 0,687, diventando il miglior risultato. 
...
L'obiettivo di questa fase è quello di rilevare, in modo indipendente dall'istanza, tutti i keypoint visibili appartenenti a qualsiasi persona dell'immagine. A tale scopo, per ogni tipologia di keypoint, viene prodotta una \emph{heatmap} e un \emph{offset}. 
...
Come abbiamo visto nel capitolo precedente, per ogni tipo di keypoint $k = 1, ..., K$, viene impostato un task di classificazione binaria come segue: generiamo una heatmap $p_k(x)$ tale che $p_k(x) = 1$ se $x \in D_R (y_{j,k})$ per qualsiasi istanza $j$, altrimenti $p_k(x) = 0$. 
...
Per quanto riguarda la stima della posa umana in un'immagine, Detectron2 utilizza una Mask R-CNN \cite{maskRCNN} riadattata all'estrazione dei keypoint.
...
\begin{itemize}
\item \textbf{Livelli} - In prima battuta è stato scelto di utilizzare una rete neurale composta da un solo livello LSTM ed un livello completamente connesso per l'inferenza finale e di incrementare il numero di livelli LSTM progressivamente fino al raggiungimento ottimale dei risultati. 
...
\item \textbf{Batch size} - La batch size, vista la dimensione del dataset ed il numero di categorie, è stato scelto di fissarla pari a $10\times N$,  dove $N$ è il numero di categorie del dataset, di modo da consentire ad ogni iterazione una sufficiente variabilità del batch ed una corretta generalizzazione nell'aggiornamento dei pesi. Per l'allenamento della rete sul dataset completo, la batch size è stata quindi fissata a $600$.
\item \textbf{Ottimizzatore} - Per la fase iniziale degli esperimenti, ogni tecnica è stata testata utilizzando  \emph{RMSProp} \cite{RMSProp} come ottimizzatore, con un valore $\rho = 0.9$ ed un learning rate iniziale di $0.01$ decrementato gradualmente fino a $0$ dopo $100$ epoche. Per le tecniche più promettenti e dopo aver stabilito la struttura della rete, è stato rimosso il vincolo delle $100$ epoche ed utilizzato un learning rate costante di $0,001$.
...
\item Categorizzare le azioni utilizzando semplicemente i valori della stima della posa ottenuti da Detectron2 e Posenet è decisamente non sufficiente, ottenendo un'accuratezza massima del $42,21\%$ (in media)
...
Come si può evincere dai risultati in tabella \ref{tab:incrementoLivelli}, incrementare la profondità della rete a due livelli spesso aiuta il processo di classificazione delle azioni, mentre aggiungerne un terzo sembrerebbe peggiorare i risultati, probabilmente a causa di un maggiore overfitting sui dati di allenamento.
...
I risultati ottenuti in questa fase sono elencati in tabella \ref{tab:dropoutERegolarizzatore}


\section{Fase 4: Allenamento completo tecnica migliore}
Quest'ultima fase è dedicata a finalizzare la miglior tecnica trovata, rimuovendo il vincolo delle 100 epoche, fissando il learning rate a $0,001$ e interrompendo l'allenamento al raggiungimento del miglior valore di accuratezza sul dataset.
Ci sono volute circa [...] epoche per raggiungere il miglior valore e i risultati ottenuti al termine di questo lungo procedimento sono i seguenti:
...
Per tale scopo abbiamo deciso di ideare un algoritmo che non avesse bisogno di informazioni iniziali complesse, come ad esempio la posizione dei giunti dei soggetti inquadrati, ma che attraverso l'uso dei soli video RGB fosse in grado di estrapolare tutte le informazioni necessarie.
...
\begin{itemize}
\item il classificatore ``\emph{3BAR-Detectron}" con un accuratezza CS: 90\% e CV 80\%
\item il classificatore ``\emph{3BAR-Posenet}" con un accuratezza CS: -\% e CV -\%
\end{itemize}
...
Chained  \cite{Chained} 					&   & \checkmark  & & -     	&   80.8 \\ \hline
ST-GCN \cite{ST-GCN}   					& \checkmark  &   & & 88.3     &   81.5 \\ \hline
\rowcolor{highlightColor} \textbf{3-BAR-PoseNet} 	& &  \checkmark  & & \textbf{-}     &   \textbf{-} \\ \hline
\rowcolor{highlightColor} \textbf{3-BAR-Detectron} 	& &  \checkmark  & & \textbf{89.39}     &   \textbf{80.88} \\ \hline
SR-TSL \cite{SR-TSL} 					& \checkmark  &   & & 92.4     &   84.8 \\ \hline
2D-3D-Softargmax \cite{2D-3D-Softargmax} 	&   & \checkmark  & & -     	&   85.5 \\ \hline
Glimpse Clouds \cite{GlimpseClouds}		&   & \checkmark & & 93.2     &   86.6 \\ \hline 
PB-GCN  \cite{PB-GCN}					& \checkmark  &  & & 93.2     &   87.5 \\ \hline 
...
\bibitem{NTURGB120} NTU RGB+D 120: A Large-Scale Benchmark for 3D Human Activity Understanding - \emph{Jun Liu, Amir Shahroudy, Mauricio Perez, Gang Wang, Ling-Yu Duan and Alex C. Kot} - 2019
...
\bibitem{MSR} Mining actionlet ensemble for action recognition with depth cameras - \emph{J. Wang, Z. Liu, Y. Wu, and J. Yuan.} - 2012

\bibitem{NUCLA} Cross-view action modeling, learning and recognition - \emph{J. Wang, X. Nie, Y. Xia, Y. Wu, and S. Zhu.} - 2014
